...
(async () => {
  try {
    ...
    for (let i = 0; i < numberOfContracts; i++) {
      ...
      const contractStorageSizeBefore = await web3.eth.getStorageAt(titleDeployingContractAddress, 2);
      ...
      const contractStorageSizeAfter = await web3.eth.getStorageAt(titleDeployingContractAddress, 2);
      const storageGrowth = parseInt(contractStorageSizeAfter, 16) - parseInt(contractStorageSizeBefore, 16);
